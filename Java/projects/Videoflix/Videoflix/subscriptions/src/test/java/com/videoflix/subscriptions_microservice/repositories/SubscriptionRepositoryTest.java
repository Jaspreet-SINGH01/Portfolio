...
public class SubscriptionRepositoryTest {
    ...
    void findByUser_shouldReturnSubscriptionsForGivenUser() {
        ...
        createAndPersistSubscription(user1, SubscriptionStatus.ACTIVE, LocalDateTime.now().minusDays(30),
                LocalDateTime.now(), null, LocalDateTime.now().plusMonths(1), LocalDateTime.now().minusMonths(1),
                LocalDateTime.now().minusDays(5), LocalDateTime.now().minusDays(1));
        // Subscription subscription2 = createAndPersistSubscription(user2,
        // SubscriptionStatus.PENDING, LocalDateTime.now().minusDays(15),
        // LocalDateTime.now().plusDays(15), null, LocalDate.now().plusMonths(2),
        // LocalDateTime.now().minusMonths(2), LocalDateTime.now().minusDays(10),
        ...
        Subscription subscription3 = createAndPersistSubscription(user1, SubscriptionStatus.CANCELLED,
                ...
                LocalDateTime.now().minusMonths(3), LocalDateTime.now().minusMonths(1),
        ...
        assertTrue(user1Subscriptions.stream().anyMatch(sub -> sub.getStatus() == SubscriptionStatus.CANCELLED));
    ...
    void countByStatus_shouldReturnCountOfSubscriptionsWithGivenStatus() {
        ...
        User user1 = createAndPersistUser(1L);
        User user2 = createAndPersistUser(2L);
        createAndPersistSubscription(user1, SubscriptionStatus.ACTIVE, LocalDateTime.now().minusDays(30), LocalDateTime.now(),
        ...
        createAndPersistSubscription(user2, SubscriptionStatus.PENDING, LocalDateTime.now().minusDays(15),
                LocalDateTime.now().plusDays(15), null, LocalDateTime.now().plusMonths(2), LocalDateTime.now(), null, null);
    ...
    void countByStartDateBetween_shouldReturnCountOfSubscriptionsWithinGivenDateRange() {
        ...
        // début.
        User user1 = createAndPersistUser(1L);
        createAndPersistSubscription(user1, SubscriptionStatus.ACTIVE, LocalDateTime.now().minusDays(5).toLocalDate(),
                LocalDateTime.now(), null, null, LocalDateTime.now(), null, null);
        createAndPersistSubscription(user1, SubscriptionStatus.PENDING, LocalDateTime.now().minusDays(1).toLocalDate(),
                LocalDateTime.now(), null, null, LocalDateTime.now(), null, null);
        createAndPersistSubscription(user1, SubscriptionStatus.CANCELLED,
                LocalDateTime.now().minusDays(10).toLocalDate(), LocalDateTime.now(), null, null, LocalDateTime.now(),
                null, null);
    ...
    private void createAndPersistSubscription(User user1, SubscriptionStatus cancelled, LocalDate localDate,
            LocalDateTime now, Object trialEndDate, Object nextBillingDate, LocalDateTime now2, Object lastPaymentDate,
            Object lastActivity) {
    ...
    void findByStatusAndStartDateLessThanEqualAndEndDateGreaterThanEqual_shouldReturnActiveSubscriptionsWithinGivenPeriod() {
        // GIVEN : Création et persistence d'abonnements avec différentes dates et
        // statuts.
        User user1 = createAndPersistUser(1L);
        createAndPersistSubscription(user1, SubscriptionStatus.ACTIVE, LocalDateTime.now().minusDays(10).toLocalDate(),
                        LocalDateTime.now().plusDays(10).toLocalDate(), null, null, LocalDateTime.now(), null, null);
                createAndPersistSubscription(user1, SubscriptionStatus.ACTIVE, LocalDateTime.now().plusDays(1).toLocalDate(),
                        LocalDateTime.now().plusDays(15).toLocalDate(), null, null, LocalDateTime.now(), null, null);
                createAndPersistSubscription(user1, SubscriptionStatus.CANCELLED,
                        LocalDateTime.now().minusDays(5).toLocalDate(), LocalDateTime.now().minusDays(1).toLocalDate(), null,
                ...
                // WHEN : Recherche des abonnements actifs dont la période chevauche la plage
                // donnée.
                List<Subscription> activeWithinPeriod = subscriptionRepository
                        .findByStatusAndStartDateLessThanEqualAndEndDateGreaterThanEqual(SubscriptionStatus.ACTIVE, endDateTime,
                                startDateTime);
        
                // THEN : Vérification que la liste contient l'abonnement attendu.
                assertEquals(1, activeWithinPeriod.size());
                assertTrue(activeWithinPeriod.stream()
                        .anyMatch(sub -> sub.getStatus() == SubscriptionStatus.ACTIVE
                                && sub.getStartDate().isBefore(startDateTime.toLocalDate())
                                && sub.getEndDate().isAfter(endDateTime.toLocalDate())));
            }
        
            private void createAndPersistSubscription(User user1, SubscriptionStatus active, LocalDate localDate,
                    LocalDate localDate2, Object trialEndDate, Object nextBillingDate, LocalDateTime now,
                    Object lastPaymentDate, Object lastActivity) {
                throw new UnsupportedOperationException("Unimplemented method 'createAndPersistSubscription'");
            }
        
            private void createAndPersistSubscription(User user1, SubscriptionStatus active, LocalDateTime localDate,
            LocalDate localDate2, Object trialEndDate, Object nextBillingDate, LocalDateTime now,
            Object lastPaymentDate, Object lastActivity) {
        throw new UnsupportedOperationException("Unimplemented method 'createAndPersistSubscription'");
    }
    ...
    void countByStatusAndStartDateLessThan_shouldReturnCountOfActiveSubscriptionsStartingBeforeGivenDate() {
        ...
        User user1 = createAndPersistUser(1L);
        createAndPersistSubscription(user1, SubscriptionStatus.ACTIVE, LocalDateTime.now().minusDays(10), LocalDateTime.now(),
                null, null, LocalDateTime.now(), null, null);
        createAndPersistSubscription(user1, SubscriptionStatus.ACTIVE, LocalDateTime.now().plusDays(1), LocalDateTime.now(),
                null, null, LocalDateTime.now(), null, null);
        LocalDateTime startDateTime = LocalDateTime.now();

        // WHEN : Comptage des abonnements actifs dont la date de début est antérieure à
        // la date donnée.
        long count = subscriptionRepository.countByStatusAndStartDateLessThan(SubscriptionStatus.ACTIVE, startDateTime);
    ...
    void findByEndDateBeforeAndStatusNot_shouldReturnNonExpiredSubscriptionsEndingBeforeGivenDate() {
        ...
        List<Subscription> endingBeforeAndNotExpired = subscriptionRepository.findByEndDateBeforeAndStatusNot(today,
                expiredStatus);

        // THEN : Vérification que la liste contient les abonnements attendus.
        assertEquals(2, endingBeforeAndNotExpired.size());
        assertTrue(endingBeforeAndNotExpired.stream().anyMatch(sub -> sub.getStatus() == SubscriptionStatus.ACTIVE));
        assertTrue(endingBeforeAndNotExpired.stream().anyMatch(sub -> sub.getStatus() == SubscriptionStatus.CANCELLED));
        assertFalse(endingBeforeAndNotExpired.stream().anyMatch(sub -> sub.getStatus() == SubscriptionStatus.EXPIRED));
    }
    ...
    void findByTrialEndDate_shouldReturnSubscriptionsWithTrialEndingOnGivenDate() {
        ...
        createAndPersistSubscription(user1, SubscriptionStatus.TRIAL, LocalDateTime.now().minusDays(10),
                LocalDateTime.now().plusDays(7), LocalDateTime.now(), null, LocalDateTime.now(), null, null);
        createAndPersistSubscription(user1, SubscriptionStatus.ACTIVE, LocalDateTime.now().minusDays(5),
                LocalDateTime.now().plusDays(10), LocalDateTime.now().plusDays(1), null, LocalDateTime.now(), null, null);
        LocalDateTime today = LocalDateTime.now();
    ...
    void findByTrialEndDateBeforeAndStatus_shouldReturnTrialSubscriptionsEndingBeforeGivenDate() {
        ...
        createAndPersistSubscription(user1, SubscriptionStatus.TRIAL, LocalDateTime.now().minusDays(15),
                LocalDateTime.now().plusDays(5), LocalDateTime.now().minusDays(1), null, LocalDateTime.now(), null, null);
        createAndPersistSubscription(user1, SubscriptionStatus.ACTIVE, LocalDateTime.now().minusDays(10),
                LocalDateTime.now().plusDays(10), LocalDateTime.now(), null, LocalDateTime.now(), null, null);
        LocalDateTime today = LocalDateTime.now();
        ...
        List<Subscription> trialEndingBeforeToday = subscriptionRepository.findByTrialEndDateBeforeAndStatus(today,
                trialStatus);

        // THEN : Vérification que la liste contient l'abonnement attendu.
        assertEquals(1, trialEndingBeforeToday.size());
        assertTrue(trialEndingBeforeToday.stream()
                .anyMatch(sub -> sub.getStatus() == SubscriptionStatus.TRIAL && sub.getTrialEndDate().isBefore(today)));
    }
    ...
    void findByStatusAndCancellationDateBefore_shouldReturnCancelledSubscriptionsCancelledBeforeGivenDate() {
        ...
        User user1 = createAndPersistUser(1L);
        Subscription subscription1 = createAndPersistSubscription(user1, SubscriptionStatus.CANCELLED, LocalDateTime.now().minusMonths(2), LocalDateTime.now().minusMonths(1), null, null, LocalDateTime.now(), null, null);
        subscription1.setCancelledAt(LocalDateTime.now().minusDays(10));
        entityManager.persist(subscription1);
        Subscription subscription2 = createAndPersistSubscription(user1, SubscriptionStatus.ACTIVE, LocalDateTime.now().minusDays(30), LocalDateTime.now(), null, null, LocalDateTime.now(),  null, null);
        ...
        List<Subscription> cancelledBeforeThreshold = subscriptionRepository.findByStatusAndCancellationDateBefore(cancelledStatus, archiveThreshold);

        // THEN : Vérification que la liste contient l'abonnement attendu.
        assertEquals(1, cancelledBeforeThreshold.size());
        assertTrue(cancelledBeforeThreshold.stream().anyMatch(sub -> sub.getStatus() == SubscriptionStatus.CANCELLED && sub.getCancelledAt().isBefore(archiveThreshold)));
    }
    ...
    void findInactiveBefore_shouldReturnInactiveSubscriptionsBeforeGivenDate() {
        ...
        Subscription subscription1 = createAndPersistSubscription(user1, SubscriptionStatus.CANCELLED,
                LocalDateTime.now().minusMonths(3), LocalDateTime.now().minusMonths(2), null, null, LocalDateTime.now(), null,
                LocalDateTime.now().minusMonths(4));
        entityManager.persist(subscription1);
        Subscription subscription2 = createAndPersistSubscription(user1, SubscriptionStatus.ACTIVE,
                LocalDateTime.now().minusDays(30), LocalDateTime.now(), null, null, LocalDateTime.now(), null,
                LocalDateTime.now().minusDays(2));
        entityManager.persist(subscription2);
        ...
        // suppression.
        List<Subscription> inactiveBeforeThreshold = subscriptionRepository.findInactiveBefore(deletionThreshold);

        // THEN : Vérification que la liste contient l'abonnement attendu.
        assertEquals(1, inactiveBeforeThreshold.size());
        assertTrue(inactiveBeforeThreshold.stream().anyMatch(sub -> sub.getStatus() == SubscriptionStatus.CANCELLED
                && sub.getLastActivity().isBefore(deletionThreshold.atStartOfDay())));
        assertFalse(inactiveBeforeThreshold.stream().anyMatch(sub -> sub.getStatus() == SubscriptionStatus.ACTIVE));
    }
    ...
    void countByCreationTimestampBetween_shouldReturnCountOfSubscriptionsCreatedWithinGivenTimeRange() {
        ...
        LocalDateTime endOfYesterday = now.minusDays(1).withHour(23).withMinute(59).withSecond(59).withNano(999999999);TimeTime
        createAndPersistSubscription(user1, SubscriptionStatus.ACTIVE, LocalDateTime.now().minusDays(2), LocalDateTime.now(),
                null, null, startOfYesterday.plusHours(1), null, null);
        createAndPersistSubscription(user1, SubscriptionStatus.PENDING, LocalDateTime.now().minusDays(1), LocalDateTime.now(),
                null, null, now.minusDays(1).withHour(12), null, null);
        createAndPersistSubscription(user1, SubscriptionStatus.CANCELLED, LocalDateTime.now().minusDays(3), LocalDateTime.now(),
    ...
    void countByStatusAndCancellationDateBetween_shouldReturnCountOfCancelledSubscriptionsCancelledWithinGivenDateRange() {
        ...
        User user1 = createAndPersistUser(1L);
        Subscription subscription1 = createAndPersistSubscription(user1, SubscriptionStatus.CANCELLED,
                LocalDateTime.now().minusMonths(2), LocalDateTime.now().minusMonths(1), null, null, LocalDateTime.now(), null,
                null);
        ...
        Subscription subscription3 = createAndPersistSubscription(user1, SubscriptionStatus.ACTIVE,
                LocalDateTime.now().minusDays(30), LocalDateTime.now(), null, null, LocalDateTime.now(), null, null);
    ...
    void findByStatusAndLastPaymentDateBetween_shouldReturnActiveSubscriptionsWithLastPaymentWithinGivenDateRange() {
        ...
        // paiement.
        User user1 = createAndPersistUser(1L);
        createAndPersistSubscription(user1, SubscriptionStatus.ACTIVE, LocalDateTime.now().minusMonths(1), LocalDateTime.now(),
        ...
        createAndPersistSubscription(user1, SubscriptionStatus.CANCELLED, LocalDateTime.now().minusMonths(2),
                LocalDateTime.now().minusMonths(1), null, null, LocalDateTime.now(), LocalDateTime.now().minusDays(2), null);
        ...
        assertTrue(activeWithPaymentInRange.stream()
                .anyMatch(sub -> sub.getPaymentDate().isAfter(minusDays.atStartOfDay())
                        && sub.getPaymentDate().isBefore(localDate.plusDays(1).atStartOfDay())));
    }
    ...
    void findSubscriptionsUpdatedSince_shouldReturnSubscriptionsUpdatedAfterGivenTimestampWithPagination() {
        ...
        createAndPersistSubscription(user1, SubscriptionStatus.ACTIVE, LocalDateTime.now().minusDays(5), LocalDateTime.now(), null, null, LocalDateTime.now(), null, LocalDateTime.now().minusDays(1));
        Subscription subscription2 = createAndPersistSubscription(user1, SubscriptionStatus.PENDING, LocalDateTime.now().minusDays(3), LocalDateTime.now(), null, null, LocalDateTime.now(), null, LocalDateTime.now().minusDays(3));
        entityManager.persist(subscription2);
        Subscription subscription3 = createAndPersistSubscription(user1, SubscriptionStatus.CANCELLED, LocalDateTime.now().minusDays(7), LocalDateTime.now(), null, null, LocalDateTime.now(), null, LocalDateTime.now());
        entityManager.persist(subscription3);

        // WHEN : Recherche des abonnements mis à jour après le timestamp donné avec pagination.
        List<Subscription> updatedSubscriptions = subscriptionRepository.findSubscriptionsUpdatedSince(lastSyncTimestamp, 0, 10);

        // THEN : Vérification que la liste contient les abonnements mis à jour après le timestamp.
        assertEquals(2, updatedSubscriptions.size());
        assertTrue(updatedSubscriptions.stream().anyMatch(sub -> sub.getLastActivity().isAfter(lastSyncTimestamp)));
        assertTrue(updatedSubscriptions.stream().anyMatch(sub -> sub.getStatus() == SubscriptionStatus.ACTIVE));
        assertTrue(updatedSubscriptions.stream().anyMatch(sub -> sub.getStatus() == SubscriptionStatus.CANCELLED));
        assertFalse(updatedSubscriptions.stream().anyMatch(sub -> sub.getStatus() == SubscriptionStatus.PENDING));
    }
    ...
    void findByStatusAndLastActivityBefore_shouldReturnSubscriptionsWithLastActivityBeforeGivenTimeWithPagination() {
        // GIVEN : Création d'abonnements avec différentes dates de dernière activité et statuts.
        User user1 = createAndPersistUser(1L);
        LocalDateTime lastActivityBefore = LocalDateTime.now().minusDays(1);
        createAndPersistSubscription(user1, SubscriptionStatus.INACTIVE, LocalDateTime.now().minusDays(10), LocalDateTime.now(), null, null, LocalDateTime.now(), null, LocalDateTime.now().minusDays(2));
        createAndPersistSubscription(user1, SubscriptionStatus.INACTIVE, LocalDateTime.now().minusDays(5), LocalDateTime.now(), null, null, LocalDateTime.now(), null, LocalDateTime.now().minusDays(3));
        createAndPersistSubscription(user1, SubscriptionStatus.ACTIVE, LocalDateTime.now().minusDays(2), LocalDateTime.now(), null, null, LocalDateTime.now(), null, LocalDateTime.now());
        Pageable pageable = PageRequest.of(0, 1);
        Subscription.SubscriptionStatus inactiveStatus = Subscription.SubscriptionStatus.INACTIVE;

        // WHEN : Recherche des abonnements inactifs dont la dernière activité est antérieure à la date donnée avec pagination.
        List<Subscription> inactiveBeforeTime = subscriptionRepository.findByStatusAndLastActivityBefore(inactiveStatus, lastActivityBefore, pageable);

        // THEN : Vérification que la liste contient le nombre attendu d'éléments pour la page demandée.
        assertEquals(1, inactiveBeforeTime.size());
        assertTrue(inactiveBeforeTime.stream().anyMatch(sub -> sub.getStatus() == Subscription.SubscriptionStatus.INACTIVE && sub.getLastActivity().isBefore(lastActivityBefore)));
    }
    ...
    void findByNextBillingDate_shouldReturnSubscriptionsWithGivenNextBillingDate() {
        // GIVEN : Création d'abonnements avec différentes dates de prochaine facturation.
        User user1 = createAndPersistUser(1L);
        LocalDateTime reminderDate = LocalDateTime.now().plusDays(7);
        createAndPersistSubscription(user1, SubscriptionStatus.ACTIVE, LocalDateTime.now().minusDays(30), LocalDateTime.now().plusMonths(1), null, reminderDate, LocalDateTime.now(), null, null);
        createAndPersistSubscription(user1, SubscriptionStatus.PENDING, LocalDateTime.now().minusDays(15), LocalDateTime.now().plusDays(15), null, LocalDateTime.now().plusDays(10), LocalDateTime.now(), null, null);

        // WHEN : Recherche des abonnements dont la prochaine date de facturation correspond à la date de rappel.
        ...
        assertTrue(billingOnReminderDate.stream().anyMatch(sub -> sub.getNextBillingDate().isEqual(reminderDate)));
    }
    ...
    void findByEndDateBeforeAndStatusIn_shouldReturnSubscriptionsEndingBeforeGivenDateAndWithGivenStatuses() {
        ...
        LocalDateTime eq = today.minusDays(0).atStartOfDay(); // Equivalent to today at 00:00:00

        // WHEN : Recherche des abonnements se terminant avant aujourd'hui et ayant les
        // statuts spécifiés.
        List<Object> foundSubscriptions = (List<Object>) subscriptionRepository.findByEndDateBeforeAndStatusIn(eq,
                statuses);
        ...
        assertTrue(foundSubscriptions.stream().allMatch(sub -> ((Subscription) sub).getEndDate().isBefore(today)));
    }
}